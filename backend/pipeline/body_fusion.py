...
# Face Mesh for eye/smile scoring (keep existing indices)
LEFT_EYE = [33, 133, 160, 158, 144, 153]
RIGHT_EYE = [362, 263, 387, 385, 380, 373]
MOUTH = [61, 291, 13, 14]

# ============================================================
# 1. REGION DEFINITION (via mask slicing)
# ============================================================
class BodyRegionParser:
    """Splits a dense silhouette into body parts using pose landmarks."""

    @staticmethod
    def split_mask(silhouette: np.ndarray, landmarks, img_shape: Tuple[int, int]) -> Dict[str, np.ndarray]:
        """
        Takes a dense silhouette mask and splits it into 13 regions.
        Returns dict: region_name -> binary mask (uint8, 0/255)
        """
        h, w = img_shape[:2]
        mask = silhouette.copy()
        regions = {}

        # Extract landmark y-coordinates (normalized 0-1)
        lms = {idx: np.array([landmarks[idx].x * w, landmarks[idx].y * h]) for idx in range(33)}

        # ---------- Vertical cut positions ----------
        ...
        # 10. Left Thigh (hips to knees, left of center)
        regions["left_thigh"] = slice_mask(hip_y, (hip_y + knee_y)/2, 0, torso_center_x)

        # 11. Left Calf (knees to ankles)
        ...
        # ---------------------- OVERLAP BUFFER (10px) ----------------------
        # To avoid seams, dilate every region slightly so they overlap at boundaries
...
class NoGapFusionEngine:
    ...
    def process_images(self, image_paths: List[str]) -> str:
        # 1. Load images, detect pose & face
        image_data = []
        ...
        # 2. Base image (best pose confidence)
        ...
        # 3. Parse all images into regions
        ...
        # 4. Score each region across all images
        scorer = RegionScorer()
        ...
        for i, data in enumerate(image_data):
            ...
            for name in region_names:
                ...
                if np.sum(mask) < 200:
                    ...
                    # Sharpness for all regions except specific face features
                    if name == "head":
                        # Combine sharpness + eyes + smile
        ...
        # 6. Composite: start with base image
        composite = base_img.copy()

        # 7. Blend each best region (with dilation already applied in parser)
        for name, idx in best_idx.items():
            ...
            # Crop to base silhouette to avoid background
        ...
        # 8. Safety net: fill any zero pixels with base image
