...
@router.post("/extract-top-frames-gallery")
async def extract_top_frames_gallery(
    ...
    try:
        ...
        frame_results = extract_best_frames_with_scores(tmp_path, num_frames=num_frames)

        frames_data = []
        for i, (score, path) in enumerate(frame_results):
            img = cv2.imread(path)
            if img is None:
                continue
...
async def reconstruct_from_parts(images: list[UploadFile] = File(...)):
    try:
        ...
        return Response(content=open(output_path, "rb").read(), media_type="image/jpeg")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
